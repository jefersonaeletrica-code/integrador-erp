...
let config = fs.existsSync(CONFIG_FILE) ? JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf8')) : { CLIENT_ID: '', CLIENT_SECRET: '', REDIRECT_URI: '', LOJA_API_URL: '', LOJA_API_KEY: '' };
...
app.post('/api/config', (req, res) => { saveConfig(req.body); res.json({ sucesso: true }); });

app.get('/api/auth/bling', (req, res) => {
    const state = Math.random().toString(36).substring(7);
    const authUrl = `https://www.bling.com.br/Api/v3/oauth/authorize?response_type=code&client_id=${config.CLIENT_ID}&redirect_uri=${encodeURIComponent(config.REDIRECT_URI)}&state=${state}`;
...
app.get('/callback', async (req, res) => {
    ...
    } catch (e) { res.status(500).send('Erro autorização: ' + e.message); }
});
